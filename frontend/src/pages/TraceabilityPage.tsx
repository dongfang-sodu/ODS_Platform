...
export function TraceabilityPage() {
  ...
  const [notice, setNotice] = useState(() => tr('Loading trace data…', '正在加载追溯数据…'))
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setNotice(tr(`Loaded ${loadedArtifacts.length} artifacts and ${loadedRelations.length} relations`, `已加载 ${loadedArtifacts.length} 个工件和 ${loadedRelations.length} 条关系`))
    } catch (error) {
  ...
  const runQuery = async () => {
    ...
    try {
      ...
      setNotice(tr(`Query complete: ${result.nodes.length} nodes in ${result.durationMs} ms`, `查询完成：找到 ${result.nodes.length} 个节点，用时 ${result.durationMs} ms`))
    } catch (error) {
  ...
  const runImpact = async (event: FormEvent) => {
    ...
    try {
      ...
      setReport(result)
      setNotice(tr(`Impact analysis complete: ${result.candidateCount} candidates require review`, `影响分析完成：生成 ${result.candidateCount} 个待复核候选`))
    } catch (error) {
  ...
  const reviewCandidate = async (candidateId: string, status: ReviewStatus) => {
    if (!report) return
    const comment = window.prompt(tr('Review note (optional)', '填写复核说明（可留空）'), status === 'CONFIRMED' ? tr('Confirmed for review', '确认需要复核') : tr('No direct impact from this change', '与本次变更无直接影响')) ?? ''
    try {
  ...
  const createTickets = async () => {
    ...
    try {
      ...
      setNotice(tr(`${candidateIds.length} linked tickets created`, `已创建 ${candidateIds.length} 张关联工单`))
    } catch (error) {
  ...
  return <>
    <PageHeader eyebrow={tr('Engineering Traceability', '工程工件追溯')} title={tr('Artifact traceability and change impact analysis', '工件追溯与变更影响分析')} description={tr('Use bounded relationship queries, explainable impact ranking and human review to connect AEB changes with projects, vehicle documentation, training, guidelines and tickets.', '使用有限深度关系查询、可解释影响排序和人工确认，把 AEB 变更连接到项目、车型说明、培训、指南和工单。')} actions={<button className="button secondary" disabled={loading} onClick={refresh}>↻ {tr('Refresh data', '刷新数据')}</button>} />
    <div className="notice-bar"><span className="live-dot" />{notice}<span className="refresh-label">{tr('Single-user demo · default depth 3 · maximum depth 5', '单用户演示 · 默认深度 3 · 最大深度 5')}</span></div>
    ...
    {tab === 'artifacts' && <div className="trace-layout">
      {canMaintain && <form className="card form-card trace-form" onSubmit={createArtifact}>
       ...
       <section className="card table-card trace-wide"><div className="table-toolbar"><div><h2>工件库</h2><p>稳定工件身份与当前不可变版本</p></div><span className="role-pill">{artifacts.length} {tr('items', '项')}</span></div><div className="table-wrap"><table><thead><tr><th>工件</th><th>类型</th><th>来源</th><th>当前版本</th><th>状态</th></tr></thead><tbody>{artifacts.map((artifact) => <tr key={artifact.id}><td><strong>{artifact.currentVersion.displayName}</strong><span className="cell-muted">{artifact.sourceObjectId}</span></td><td>{artifact.type.name}</td><td>{artifact.sourceModule}</td><td>{artifact.currentVersion.versionLabel}</td><td><span className="status status-active">{label(artifact.sourceStatus)}</span></td></tr>)}</tbody></table></div></section>
    </div>}
    ...
    {tab === 'impact' && <>
      {canAnalyze ? <form className="card form-card" onSubmit={runImpact}><div className="card-heading"><h2>创建变更并生成候选</h2></div><div className="form-grid"><label className="field field-wide"><span>变更源</span><select value={impactSource} onChange={(event) => setImpactSource(event.target.value)}>{versions.map(({ artifact, version }) => <option key={version.id} value={version.id}>{artifact.type.name} · {version.displayName}</option>)}</select></label><label className="field"><span>变更类型</span><select value={changeType} onChange={(event) => setChangeType(event.target.value as ChangeType)}><option value="PARAMETER">参数变化</option><option value="HARDWARE">硬件变化</option><option value="GOAL">目标变化</option><option value="OTHER">其他变化</option></select></label><label className="field"><span>分析深度</span><select value={queryDepth} onChange={(event) => setQueryDepth(Number(event.target.value))}>{[1, 2, 3, 4, 5].map((depth) => <option key={depth}>{depth}</option>)}</select></label><label className="field"><span>变更前</span><input value={beforeContent} onChange={(event) => setBeforeContent(event.target.value)} /></label><label className="field"><span>变更后</span><input value={afterContent} onChange={(event) => setAfterContent(event.target.value)} /></label><label className="field field-wide"><span>变更说明</span><textarea required rows={3} value={changeDescription} onChange={(event) => setChangeDescription(event.target.value)} /></label></div><div className="form-actions"><button className="button primary">生成可解释影响报告</button></div></form> : <div className="card form-intro">当前角色可查看已有报告和执行追溯查询，但不能发起变更分析。</div>}
       {report && <section className="card table-card trace-report"><div className="table-toolbar"><div><h2>{report.change.sourceName}</h2><p>{label(report.change.changeType)} · {label(report.status)} · 规则 {report.scoringRuleVersion}</p></div>{canCreateTickets && <button className="button primary" onClick={createTickets}>为确认项创建工单</button>}</div>{(report.truncatedByDepth || report.truncatedByNodeLimit) && <div className="trace-warning-block">分析达到范围上限，候选结果可能不完整。</div>}<div className="table-wrap"><table><thead><tr><th>候选工件</th><th>分值/等级</th><th>系统路径</th><th>人工结论</th><th>操作</th></tr></thead><tbody>{report.candidates.map((candidate) => <tr key={candidate.id}><td><strong>{candidate.target.displayName}</strong><span className="cell-muted">{candidate.target.artifactTypeCode} · {candidate.target.sourceModule}</span>{candidate.tickets.map((ticket) => <span className="trace-ticket" key={ticket.id}>{ticket.externalKey}</span>)}</td><td><span className={`impact-level level-${candidate.initialLevel.toLowerCase()}`}>{label(candidate.initialLevel)}</span><strong className="impact-score">{Number(candidate.initialScore).toFixed(1)}</strong></td><td>{candidate.paths.map((path) => <details key={path.pathRank}><summary>{path.length}跳 · 查看解释</summary><ol>{path.steps.map((step) => <li key={step.sequenceNo}>{step.relationTypeCode} · {label(step.traversalDirection)} · 权重{Number(step.relationWeight).toFixed(2)}</li>)}</ol></details>)}</td><td><span className={`status review-${candidate.reviewStatus.toLowerCase()}`}>{label(candidate.reviewStatus)}</span>{candidate.reviewComment && <span className="cell-muted">{candidate.reviewComment}</span>}</td><td>{canReview ? <div className="trace-actions"><button className="small-button" disabled={candidate.reviewStatus === 'CONFIRMED'} onClick={() => reviewCandidate(candidate.id, 'CONFIRMED')}>确认影响</button><button className="small-button danger-button" disabled={candidate.reviewStatus === 'EXCLUDED'} onClick={() => reviewCandidate(candidate.id, 'EXCLUDED')}>排除</button></div> : '只读'}</td></tr>)}</tbody></table></div></section>}
    </>}
  </>
}
